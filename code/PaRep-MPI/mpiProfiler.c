...
void restoreStack ()
{
  if (replace (stackSegAddr [0], stackSegAddr [1] - stackSegAddr [0] + 1, 61, CMP_REP_INTERCOMM) < 0)
  {
...
{
  int rank, size, i, typeSize, src, dest;
  void *buf1, *buf2;
  MPI_Status *stats;
  ...
  if (nR > 0 && comm == MPI_COMM_WORLD)
  {
    ...
    MPI_Type_extent (sendtype, &sdextent);
    MPI_Type_extent (recvtype, &rdextent);

    requests = (MPI_Request *) myMalloc (&requests, 2 * size * sizeof (MPI_Request));
    stats = (MPI_Status *) myMalloc (&stats, 2 * size * sizeof (MPI_Status));

    for (i = 0; i < size; i++) 
    { 
      MPI_Irecv ((void *) recvbuf + i * recvcount * rdextent, recvcount, recvtype, i, 100 + rank, comm, &requests [2*i]);
      MPI_Isend ((void *) sendbuf + i * sendcount * sdextent, sendcount, sendtype, i, 100 + i, comm, &requests [2*i + 1]);
    }

    MPI_Waitall (2 * size, requests, stats);
...
int MPI_Alltoallv (void *sendbuf, int *sendcnts, int *sdispls, MPI_Datatype sendtype, void *recvbuf, int *recvcnts, 
                   int *rdispls, MPI_Datatype recvtype, MPI_Comm comm)
{

  int rank, size, i, j, src, dest, typeSize;
  void *buf1, *buf2;
  ...
  if (nR > 0 && comm == MPI_COMM_WORLD)
  {
    ...
    for (i = 0; i < size; i++) 
    { 
      MPI_Irecv ((void *) recvbuf + rdispls [i] * rdextent, recvcnts[i], recvtype, i, 200 + rank, comm, &requests [2*i]);
      MPI_Isend ((void *) sendbuf + sdispls [i] * sdextent, sendcnts[i], sendtype, i, 200 + i, comm, &requests [2*i + 1]);
    }

    MPI_Waitall (2 * j, requests, stats);
